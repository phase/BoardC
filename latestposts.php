<?php
	...
	$posts = $sql->query("
		SELECT 	p.id pid, p.time, p.thread tid, $userfields,
				t.name tname, t.id tinvchk, t.forum fid, f.id finvchk,
				f.name fname, f.minpower fpowl, $new_check new
		FROM posts p
		...
		".($isadmin ? "" : "WHERE f.minpower <= {$loguser['powerlevel']} AND NOT ISNULL(f.id)")."
		$timelimit
	...
	if ($posts){
		while($post = $sql->fetch($posts)){
			...
			<tr>
				...
				<td class='dim c'>
					<a href='forum.php?id={$post['fid']}'>
						".(
							$post['finvchk'] ?
							$post['fname'] : 
							"<div class='danger' style='background: #fff'>Invalid forum ID #{$post['fid']}</div>"
						)."
					</a>
				</td>
	...
	Show:			
	<div class='fonts'>
		Last <a href='?t=1800'>30 minutes</a> - <a href='?t=3600'>1 hour</a> - <a href='?t=18000'>5 hours</a> - <a href='?t=86400'>1 day</a><br>
		Most recent <a href='?p=20'>20 posts</a> - <a href='?p=50'>50 posts</a> - <a href='?p=100'>100 posts</a>
	</div>
	
	<table class='main w'>
	
		<tr>
			<td class='dark c' colspan=5>
				<b>Latest Posts</b>
			</td>
		</tr>
