<?php
	...
	// If you're looking for the global posts/threads, don't count manually and just use the total post counter in misc
	if ($users){
		if ($time || $pm_filter) 	$max = $sql->resultq("SELECT COUNT(id) FROM $q_table p $q_time $pm_filter");
		else						$max = $miscdata["$q_table"];
	}
	else $max = 0;
	...
	$i			= 0;
	$totalusers = 0;
	$pre 		= NULL; // Previous post counter
	...
	while($x = $sql->fetch($users)){
		...
		$txt .= "
			<tr>
				...
				<td class='dim c' width='100'>
					$width%<br>
					<img src='images/bar/minibar.png' height='3' align='left' width='$width%'>
				</td>
			</tr>
		";
